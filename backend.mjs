...
function redact(text,key){return String(text||'').split(key).join('[REDACTED]').slice(0,400);}
...
function extractSources(output){
  ...
  return [...new Map(sources.map(s=>[s.url,s])).values()].slice(0,10);
}
export async function runBackend({backend,config,history,signal,onStatus=()=>{},onTool=()=>{},onSources=()=>{},onUsage=()=>{}}){
  ...
  const tools=[...toolDefinitions,...(config.webSearch?[{type:'web_search'}]:[])];
  ...
  for(let round=0;round<5;round++){
    ...
    const response=await requestResponse(backend,{model:backend.model,instructions,input,tools,
    ...
    for(const item of response.output||[])if(item.type==='web_search_call')onTool({name:'web_search',status:'completed',summary:'已完成联网搜索'});
